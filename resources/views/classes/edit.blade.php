...
<div class="mb-8 border-b border-[#E5E3DB] pb-5"> 
    ...
    <p class="mt-1 text-sm text-slate-500">Memperbarui catatan kelas <span class="font-medium text-[#16213A]">XII AKL 1</span>.</p> 
</div> 
...
<form action="{{ route('classes.update', $class['id']) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8"> 
    ...
    <div> 
        ...
        <select id="grade" name="grade" 
            class="w-full border border-[#D9D6CD] bg-[#FCFBF8] px-3.5 py-2.5 text-sm focus:border-[#A16207] focus:bg-white focus:outline-none"> 
            <option value="X" {{ $class['grade'] === 'X' ? 'selected' : '' }}>Tingkat X</option> 
            <option value="XI" {{ $class['grade'] === 'XI' ? 'selected' : '' }}>Tingkat XI</option> 
            <option value="XII" {{ $class['grade'] === 'XII' ? 'selected' : '' }}>Tingkat XII</option> 
        </select> 
    </div> 
    ...
    <div> 
        ...
        <select id="major_id" name="major_id" 
            ...
            @foreach($majors as $major)
                <option value="{{ $major['id'] }}" {{ $class['major_id'] === $major['id'] ? 'selected' : '' }}>{{ $major['code'] }}</option>
            @endforeach
        </select> 
    </div> 
